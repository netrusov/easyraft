...
1. Build example:
   1. `docker build -t webkvs:latest .`
...
You can try scale up/down (by setting replicas for example in
`deployments.yaml`) and check if you are able to put/get data and all the nodes
are okay.

**Note:** It is recommended to use `StatefulSet` as it will nicely scale up/down
one-by-one the nodes without any issue.
